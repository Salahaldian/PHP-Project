...
{
    $id = $_GET['id'];
    $qurey = "SELECT categories_name from categories  where id = '$id'";
    $result = mysqli_query($link,$qurey);
    $row = mysqli_fetch_assoc($result);
    $name = $row['categories_name'];
    if ($_SERVER['REQUEST_METHOD'] == "POST"){
        $cate = $_POST['name'];
        $qurey = "UPDATE categories set categories_name ='$cate' where id = '$id'";
        $result = mysqli_query($link,$qurey);
        if ($result) {
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="basic-form">
                            <form class="" novalidate action="<?php echo  $_SERVER['PHP_SELF']."?id=$id" ?>" method="post">
                                <div class="mb-3">
                                    <label class="text-label form-label" >Categories</label>
                                    <div class="input-group input-group-lg">
                                        <span class="input-group-text">Add</span>
                                        <input name="name" type="text" class="form-control" placeholder="Enter a Category Name.." required value="<?php echo $name; ?>">
                                        <div class="invalid-feedback">
                                        </div>
                                    </div>
                                </div>
                                <button type="submit" class="btn me-2 btn-primary">Edit</button>
                            </form>
                        </div>
                    </div>
                </div>
